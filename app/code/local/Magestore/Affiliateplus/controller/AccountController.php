...
class Magestore_Affiliateplus_AccountController extends Mage_Core_Controller_Front_Action {
    ...
    /* 
        Changed by Tab (14/11/2016)
    */

    public function editPageBannerAction()
    {
        if(!Mage::helper('affiliateplus')->isAffiliateModuleEnabled()) return $this->_redirectUrl(Mage::getBaseUrl());
        ...
        if (!$this->getRequest()->isPost())
            return $this->_redirect('affiliateplus/index/substore');
        ...
        try {
            $data['substore_description'] = isset($data['substore_description']) ? htmlentities($data['substore_description']) : '';            
            $account->setDescription($data['substore_description']);
            $account->save();
            // \Zend_Debug::dump($account->getDescription());
            $successMessage = $this->__('Your sub store banner information has been saved.');
