...
class Magestore_Affiliateplus_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function indexAction() {
        ...
        // var_dump($this->_getCoreSession()->getData('has_been_signup'));die();
            ...
            // }
            if ($this->_getAccountHelper()->getAccount()->getApproved() == 2 && $this->_getAccountHelper()->getAccount()->getStatus() == 2){
                $this->_getCoreSession()->addNotice($this->_getHelper()->__('Your affiliate registration is awaiting for approval. Please be patient.'));
            }
            // elseif (!$this->_getCoreSession()->getData('has_been_signup')){
            //     $this->_getCoreSession()->addNotice($this->_getHelper()->__('Your affiliate registration is awaiting for approval. Please be patient.'));
            // }
            elseif ($this->_getAccountHelper()->getAccount()->getApproved() == 2 && $this->_getAccountHelper()->getAccount()->getStatus() == 3){
                $this->_getCoreSession()->addNotice($this->_getHelper()->__('Your affiliate registration has been denied. Please contact us for more informations.'));
            }
            elseif ($this->_getAccountHelper()->getAccount()->getApproved() == 1 && $this->_getAccountHelper()->getAccount()->getStatus() == 2){
                $this->_getCoreSession()->addNotice($this->_getHelper()->__('Your affiliate account is currently disabled. Please contact us to resolve this issue.'));
            }
        }
        $this->loadLayout();
...
//            $message = Mage::helper('affiliateplus')->__('You cannot add this product to your substore');
    ...
    public function removeproductAction() {
        ...
        if(Mage::helper('affiliateplus')->checkExistAccountProduct($accountId, $productId)){
            try{
                $modelCollection = Mage::getModel('affiliateplus/accountproduct')
                    ->getCollection()
                    ->addFieldToFilter('account_id', $accountId)
                    ->addFieldToFilter('product_id', $productId);
                $accountProductId = $modelCollection->getData()[0]["accountproduct_id"];
                $model = Mage::getModel('affiliateplus/accountproduct')
        ...
        if($error) {
            $html = "<div class='error-msg'>" . $this->__('You cannot remove this product from your substore') . "</div>";
        } else {
            $html = "<div class='success-msg'>" . $this->__('The product has been removed from your substore successfully.') . "</div>";
        }
        $this->getResponse()->setBody($html);
    }
}
